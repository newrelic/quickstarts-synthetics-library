...
var payload = 'MyXmlOrJsonPayload'

var SuccessCriteria= 'Success';
var options = {
    //Define endpoint URL.
...
function callback(error, response, body) {
  ...
  assert.ok(response.statusCode == 200, 'Expected 200 OK response. Please review the response: '+response.body);

	var XMLResponse = response.body;
	var status = XMLResponse.search(SuccessCriteria);
	assert.ok(status >= 0, 'Unable to find ' + SuccessCriteria +'.  Please review the response: ' +XMLResponse);
