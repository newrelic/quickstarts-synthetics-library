...
function getStartOfDST(year) {
    var date = new Date(Date.UTC(year, 2, 7, 7, 0));
    date.setDate(7 + (7 - date.getDay()));
    return date;
}

function getEndOfDST(year) {
    var date = new Date(Date.UTC(year, 9, 31, 6, 0));
...
function isInDST(someDate) {
	...
	var eDST = getEndOfDST(year);
	var sDST = getStartOfDST(year);
...
function adjustedTZDate() {
    ...
    var myOffSet = 0;                   // init offset value to zero
    var someDate = new Date();          // instantiate the date object
