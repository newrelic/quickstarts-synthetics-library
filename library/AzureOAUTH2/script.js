...
function getToken() {
  $http.post(
    `https://login.microsoftonline.com/${$secure.AZURE_TENANT}/oauth2/v2.0/token`,
    {
      headers: {
    ...
    function (err, resp) {
      ...
      assert.ok(
        resp.statusCode === 200,
        `Error response code received during oauth ${resp.statusCode}`
      );
...
function getSecret(token) {
  $http.get(
    'https://test-bpeck.vault.azure.net/secrets/SampleSecret/9870ddd28a3a428a85bda4261bf66c31?api-version=7.1',
    {
      headers: {
