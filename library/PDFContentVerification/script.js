...
$browser
  ...
  .then(() => {
    ...
    log(
      1,
      'Navigate to https://newrelic.com/resources/datasheets/new-relic-one'
    );
    return $browser.get(
      'https://newrelic.com/resources/datasheets/new-relic-one'
    );
  })
  .then(() => {
  ...
  .then((data) => {
    ...
    assert.ok(
      myRegex.test(data.text),
      `ERROR - Unable to find ${myRegex} in the PDF`
    );
  })
  .then(
    function () {
